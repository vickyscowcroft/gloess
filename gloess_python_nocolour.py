...
if nu > 0:
	...
	erru = sdevu / np.sqrt(nu)

	if nu > 1:
		print  '<U> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aveu, sdevu/np.sqrt(nu), ampu)
		print >> avsout, '<U> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aveu, sdevu/np.sqrt(nu), ampu)
...
if nb > 0:
	...
	errb = sdevb / np.sqrt(nb)

	if nb > 1:
		print  '<B> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aveb, sdevb/np.sqrt(nb), ampb)
		print >> avsout, '<B> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aveb, sdevb/np.sqrt(nb), ampb)
	if nb == 1:
...
if nv > 0:
	...
	errv = sdevv / np.sqrt(nv)

	if nv > 1:
		print >> avsout, '<V> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f} ' .format(avev, sdevv/np.sqrt(nv), ampv)
...
if nr > 0:
	...
	errr = sdevr / np.sqrt(nr)

	if nr > 1:
		print '<R> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aver, sdevr/np.sqrt(nr), ampr)
		print >> avsout, '<R> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aver, sdevr/np.sqrt(nr), ampr)
	if nr == 1:
...
if ni > 0:
	...
	erri = sdevi / np.sqrt(ni)

	if ni > 1:
		print  '<I> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(avei, sdevi/np.sqrt(ni), ampi)
		print >> avsout, '<I> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(avei, sdevi/np.sqrt(ni), ampi)
	if ni == 1:
...
if nj > 0:
	...
	errj = sdevj / np.sqrt(nj)

	if nj > 1:
		print >> avsout, '<J> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(avej, sdevj/np.sqrt(nj), ampj)
...
if nh > 0:
	...
	errh = sdevh / np.sqrt(nh)
	if nh > 1:
		print >> avsout, '<H> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(aveh, sdevh/np.sqrt(nh), amph)
...
if nk > 0:
	...
	errk = sdevk / np.sqrt(nk)

	if nk > 1:
		print >> avsout, '<K> = {0:.3f}    std dev = {1:.3f}     amplitude = {2:.3f}' .format(avek, sdevk/np.sqrt(nk), ampk)
...
if nir4 > 0:
	...
	ax1.plot(ir4x,ir41-2.6,'k-', zorder=4)
...
print >> avstable, '{0:s} {1:.4f} {2:.3f} {3:.3f} {4:.3f} {5:.3f}'.format(cepname, period, aveir1, sdevir1, aveir2, sdevir2)
avstable.close() 
